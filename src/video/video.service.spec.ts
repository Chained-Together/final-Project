...
describe('VideoService', () => {
  ...
  describe('영상 메타 데이터 저장 시 ', () => {
    it('유저가 소유한 채널이면 영상을 저장하고 반환한다.', async () => {
      ...
      expect(mockResolutionRepository.createResolution).toHaveBeenCalledWith(mockVideo);
  ...
  describe('영상 상세 조회 시', () => {
    ...
    it('공개(PUBLIC) 영상이면 메타데이터와 채널 정보를 반환한다.', async () => {
      mockResolutionRepository.findResolutionByVideoId,
        mockVideoRepository.findVideoWithChannelAndResolution.mockResolvedValue({
          ...mockVideo,
          visibility: Visibility.PUBLIC,
          resolution: { videoUrl: 'http://example.com/video.mp4' },
        });
  ...
  describe('영상 수정 시', () => {
    it('유저가 소유한 채널이 아닐 시 UnauthorizedException을 던져야 한다.', async () => {
      ...
      await expect(videoService.updateVideo(mockUser, 1, mockUpdateVideoDto)).rejects.toThrow(
        UnauthorizedException,
      );
    });
